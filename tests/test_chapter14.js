...
/**
 * Test Script for Chapter 14: Entity Authentication
 * 
 * This script verifies the correctness of entity authentication implementations
 * including password-based authentication, OTP systems, challenge-response
 * protocols, and biometric authentication metrics.
 */

import crypto from 'crypto';

console.log('🔐 Testing Chapter 14: Entity Authentication');
...
runTest('otpSystems', 'OTP authentication logic is correct', () => {
    const seed = 'testseed';
    const length = 10;
    const hashChain = [];
    ...
    if (counter > 0) {
        const expectedHash = hashChain[counter - 1];
        const providedHash = simpleHashSync(expectedHash);
...
runTest('otpSystems', 'OTP chain prevents replay attacks', () => {
    ...
    const providedHash1 = simpleHashSync(expectedHash1);
    const firstAuth = providedHash1 === currentHash;
...
runTest('biometricMetrics', 'Biometric accuracy calculations are reasonable', () => {
    ...
    for (const metric of biometricData) {
        // Accuracy should be consistent with error rates
        const expectedAccuracy = 100 - (metric.far * 100 + metric.frr * 100);
        const accuracyDiff = Math.abs(metric.accuracy - expectedAccuracy);
        
        // Allow some tolerance for different calculation methods
        if (accuracyDiff > 10) return false;
...
await runAsyncTest('securityProperties', 'Password hashing is one-way', async () => {
    ...
    // Should not be able to derive password from hash
...
runTest('securityProperties', 'OTP provides forward security', () => {
    ...
    // Knowing OTP at position i doesn't help compute OTP at position i+1
    // (This is ensured by the one-way nature of the hash function)
...
async function runAllTests() {
    // All async tests are already awaited above
    ...
    console.log('• One-time passwords using Lamport hash chains');
